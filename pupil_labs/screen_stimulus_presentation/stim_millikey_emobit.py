...
def main():
    ...
    p = PupilCore()
    # Start Millikey listening process
    millikey_proc = Process(target=millikey_listener)
    millikey_proc.start()
    ...
    # Close PsychoPy
    millikey_proc.terminate()
    millikey_proc.join()
    print("==> Listener MilliKey CLOSED.")
    core.quit()
    print("==> EXIT DONE.")    


def millikey_listener():
    import time
    from pylsl import StreamInfo, StreamOutlet
    ...
    port_name = None
    for p in ports:
        if "USB" in p.device or "tty" in p.device or "COM" in p.device:
            port_name = p.device
            break
    if not port_name:
        print(" There's not serial port for MilliKey.")
        return

    print(f"MilliKey conected at {port_name}")
    try:
        ser = serial.Serial(port_name, 115200, timeout=1)
        info = StreamInfo(name='MilliKey', type='Markers', channel_count=1,
                          channel_format='string', source_id='millikey123')
        outlet = StreamOutlet(info)

        while True:
            line = ser.readline().decode('utf-8').strip().lower()
            if line:
                print(f"Boton pushed: {line}")
                outlet.push_sample([f'MilliKey_{line}'])
    except Exception as e:
        print(f"Error > MilliKey listener: {e}")
